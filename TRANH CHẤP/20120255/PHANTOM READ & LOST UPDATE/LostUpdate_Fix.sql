...
create
--alter
proc usp_Dat_Trang_Thai_XoaBo_HD_FIX
	@iD_HD varchar(10)
as
...
begin try
	...
	--
	if exists (select * from HOPDONG where ID_HOPDONG = @iD_HD and TINHTRANG = NULL)
	begin
		print(N'Hợp đồng đã ở trạng thái này từ trước')
		rollback tran
		return 1
	end
	if exists (select * from HOPDONG with (xlock) where ID_HOPDONG = @iD_HD and datediff(day,NGAYKETTHUCDK,getdate()) < 0 and TINHTRANG = 1)
	begin
...
create
--alter
proc usp_Update_1_Hop_Dong_Chua_Duyet_Sang_Da_Duyet_FIX
	@iD_HD varchar(10)
...
begin try
	...
	if not exists (select * from HOPDONG with (xlock) where TINHTRANG = 0 and ID_HOPDONG = @iD_HD)
	begin
